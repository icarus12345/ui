  ...
  <Presence :present="forceMount || open">
    <ToastRootImpl
      ...
      @swipe-move="(event) => {
        ...
        target.style.setProperty('--ui-toast-swipe-move-x', `${x}px`);
        target.style.setProperty('--ui-toast-swipe-move-y', `${y}px`);
      }"
      @swipe-cancel="(event) => {
        const target = event.currentTarget as HTMLElement
        target.setAttribute('data-swipe', 'cancel');
        target.style.removeProperty('--ui-toast-swipe-move-x');
        target.style.removeProperty('--ui-toast-swipe-move-y');
        target.style.removeProperty('--ui-toast-swipe-end-x');
        target.style.removeProperty('--ui-toast-swipe-end-y');
      }"
      @swipe-end="(event) => {
        const { x, y } = event.detail.delta;
        const target = event.currentTarget as HTMLElement
        ...
        target.style.removeProperty('--ui-toast-swipe-move-x');
        target.style.removeProperty('--ui-toast-swipe-move-y');
        target.style.setProperty('--ui-toast-swipe-end-x', `${x}px`);
        target.style.setProperty('--ui-toast-swipe-end-y', `${y}px`);
        open = false;
      }"
    >
      <slot
